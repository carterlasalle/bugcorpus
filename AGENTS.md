...
|Area|Path|Purpose|
|---|---|---|
|Core CLI/lib|`bugcorpus/`|corpus, engines, scanner, verifier, CLI, MCP|
|Corpus state|`.bugcorpus/`|config, cases, families, detectors, schemas, generated indexes|
|Canonical skill|`skills/bug-corpus/`|shared agent workflow (adapters copy it per harness)|
|Tests|`tests/bugcorpus/`|pytest suite|
|CI|`.github/workflows/bugcorpus.yml`|verify + pr/full scans|
|Requirements spec|`docs/specs/bug-corpus.md`|trace REQ nodes for this repo|
...
uv run bugcorpus scan     # human summary; add --json for machines
```

### Focused test

```sh
uv run pytest tests/bugcorpus/test_system.py -q
uv run bugcorpus verify --detector <id>
```
...
```sh
uvx ruff@latest check bugcorpus/ tests/
uvx ruff@latest format bugcorpus/ tests/
```
...
```sh
uv build  # hatchling wheel; entrypoint bugcorpus=bugcorpus.cli:main
```

### Full validation / Definition-of-Done command

```sh
uv run pytest tests/ -q && uv run bugcorpus verify && trace verify --changed
```
...
Python 3.11+ (dev on 3.13/3.14); package manager `uv` only (no pipenv/Poetry/pip/Conda).
Runtime deps: `pyyaml`. Dev group: `pytest`. Lint via `uvx ruff`, types via system `pyright`.
...
-->
- `.bugcorpus/config.toml`: scan include/exclude, per-engine timeout, promotion thresholds, baseline file.
- `pyproject.toml`: hatchling build, `bugcorpus` entrypoint, ruff per-file-ignores for fixtures.
- `pyrightconfig.json`: excludes minimized corpus fixtures from typechecking.
- No secrets anywhere; no `.env` in this repo.
...
- `bugcorpus/` (deterministic core) never imports harness adapters; adapters call the core.
- Engine subprocesses (`engines.py`) are the only cross-process seam; findings normalize to the internal schema (SARIF is export-only).
- `.bugcorpus/` is data: code reads it, never the reverse.
...
- `bugcorpus/models.py` Dataclasses are the in-code contract; `.bugcorpus/schemas/*.json` mirror it.
- `.bugcorpus/generated/*.json` is regenerated by `store.write_index()`; never hand-edit.
- Detector manifests own rule metadata; BugCase `bug.yaml` files own case evidence; lineage lives in `catches`/`detector_ids`.
...
All pytest in `tests/bugcorpus/` (models, fingerprints, detectors on real fixtures, system e2e).
Fastest single test: `uv run pytest tests/bugcorpus/test_system.py::test_scan_finds_only_known_positives -q`.
Detector behavior is proved by `uv run bugcorpus verify` (fixture recall/precision), not by mocks.
...
- `bugcorpus scan` from a directory without `.bugcorpus/` silently scans nothing (root discovery falls back to cwd). Always run from the repo root or pass `--cwd`.
- `bugcorpus learn` in the repo creates a real `BC-NNNNNN` directory; sandbox it with `--cwd $(mktemp -d)` plus `init` when experimenting.
- Corpus fixture `.py` files intentionally contain unresolved names; lint/type configs exclude them (pyproject per-file-ignores, pyrightconfig.json).
...
-->
- Trace: in YAML files only `# trace:v1` markers attach to config-key boundaries; `# trace:exempt` does not (use node markers or `trace ignore`). In Python both forms work.
- Trace: `trace new requirement` mints an ID but persists no node; requirement nodes come from spec-doc markers (`docs/specs/*.md`) or bootstrap.
...
- Greenfield repos hit TL012/TL013 on every new file: bootstrap work first (`trace new work`), define REQs in `docs/specs/`, then mark primary boundaries and exempt helpers as you write — never batch-trace at the end.
- The BugCorpus review pass paid off 3x in one session (reassignment FP, comment/string FP, subprocess cwd). Budget an attack pass before completion.
...
2026-09-10 (Bug Corpus initial implementation)

Reviewed: WORK-BUG-ZJBDCZZ0
